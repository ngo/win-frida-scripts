...
function getNcrypt() {
    // Frida 17+: use Process.* to get a Module instance, then module.find/getExportByName()
    var m = Process.findModuleByName("ncrypt.dll");
    if (m === null) {
        console.log("ncrypt.dll is not loaded in this process (yet). Nothing to hook.");
    }
    return m;
}

function findNcryptExport(name) {
    var ncrypt = getNcrypt();
    if (ncrypt === null) return null;
    try {
        return ncrypt.findExportByName(name);
    } catch (e) {
        console.log("Error resolving export " + name + ": " + e);
        return null;
    }
}

function attachNcryptExport(name, callbacks, notFoundMsg) {
    var addr = findNcryptExport(name);
    if (addr === null) {
        console.log(notFoundMsg || (name + " export not found!"));
        return null;
    }
 ...
 */
var shh = findNcryptExport("SslHashHandshake");
if (shh !== null) {
    ...
    console.log(
        "SslHashHandshake export not found! Either using a very old windows machine or something is wrong with frida or ncrypt.dll."
    );
}
...
attachNcryptExport(
    "SslGenerateMasterKey",
    {
        onEnter: function (args) {
            ...
            this.client_random =
                parse_parameter_list(this.pParameterList, "SslGenerateMasterKey") ||
                client_randoms[this.threadId] ||
                "???";
        },
        onLeave: function (retval) {
    ...
    "SslGenerateMasterKey export not found!"
);
...
attachNcryptExport(
    "SslImportMasterKey",
    {
        onEnter: function (args) {
            ...
            this.client_random =
                parse_parameter_list(this.pParameterList, "SslImportMasterKey") ||
                client_randoms[this.threadId] ||
                "???";
        },
        onLeave: function (retval) {
    ...
    "SslImportMasterKey export not found!"
);
...
attachNcryptExport(
    "SslGenerateSessionKeys",
    {
        onEnter: function (args) {
            ...
            this.client_random =
                parse_parameter_list(this.pParameterList, "SslGenerateSessionKeys") ||
                client_randoms[this.threadId] ||
                "???";
    ...
    "SslGenerateSessionKeys export not found!"
);
...
var setk = findNcryptExport("SslExpandTrafficKeys");
if (setk !== null) {
    Interceptor.attach(setk, {
        ...
        onLeave: function (retval) {
            ...
            keylog(
                "CLIENT_" + this.suffix + " " + this.client_random + " " + buf2hex(key1)
            );
            keylog(
                "SERVER_" + this.suffix + " " + this.client_random + " " + buf2hex(key2)
            );
        },
    });
} else {
    console.log(
        "SslExpandTrafficKeys export not found! Probably using an old OS such as Win 7. Otherwise report an issue."
    );
}

var seemk = findNcryptExport("SslExpandExporterMasterKey");
if (seemk !== null) {
    ...
    console.log(
        "SslExpandExporterMasterKey export not found! Probably using an old OS such as Win 7. Otherwise report an issue."
    );
}
